...
def _build_parser() -> argparse.ArgumentParser:
    ...
    hw.add_argument("--device", type=int, default=0, metavar="N",
                    help="RTL-SDR device index")
    hw.add_argument("--no-bias-tee", action="store_true",
                    help="Do NOT enable the bias tee (use only with passive antenna or external LNA)")
    hw.add_argument("--ppm", type=int, default=0, metavar="PPM",
                    help="RTL-SDR frequency correction in parts-per-million")
    hw.add_argument("--gain", type=float, default=49.6, metavar="DB",
                    help="RTL-SDR gain in dB (49.6 = max)")
    hw.add_argument("--sample-rate", type=float, default=2.048, metavar="MSPS",
    ...
    acq.add_argument("--doppler-step", type=float, default=500, metavar="HZ",
                     help="Doppler bin step in Hz")
    acq.add_argument("--integration-ms", type=int, default=1, metavar="MS",
    ...
    run.add_argument("--simulate", action="store_true",
                     help="Use synthetic signal (no hardware needed)")
    run.add_argument("--once", action="store_true",
                     help="Run a single acquisition pass then exit")
    run.add_argument("--json", action="store_true",
                     help="Machine output: suppress tables/preamble, emit one JSON "
                          "document per sweep on stdout (JSONL in loop mode)")
    run.add_argument("--interval", type=float, default=5.0, metavar="SEC",
...
def _render_table_rich(
    ...
    header_lines = [
        ...
        f"Mode: [{'green' if mode == 'HARDWARE' else 'magenta'}]{mode}[/]  "
        f"Bias tee: [{'green' if bias_tee_ok else 'red'}]{'ON ✓' if bias_tee_ok else 'OFF ✗'}[/]  "
        f"Satellites acquired: [{'green' if acquired else 'yellow'}]{len(acquired)} / {len(results)}[/]",
    ]
...
def _render_table_plain(
    ...
    print(f"Bias tee: {'ON' if bias_tee_ok else 'OFF'}  │  Acquired: {len(acquired)}/{len(results)}")
    print(f"{'─'*70}")
    ...
    for r in results:
        ...
        print(f"{r.prn:>4}  {status:^10}  {r.doppler_hz:>+9.0f}Hz  {r.code_phase:>6}  "
              f"{r.metric:>7.2f}  {bar}")
    print(f"{'─'*70}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    from .acquisition import GPSAcquisition
    ...
    if not quiet:
        print("ready.")
        print(f"  {len(acq.doppler_bins)} Doppler bins  ×  32 PRNs  "
    ...
    if args.simulate:
        # ── simulation mode ───────────────────────────────────────────────────
        from .simulate import DEFAULT_SCENARIO, GPSSimulator
        ...
        if not quiet:
            ...
            for s in DEFAULT_SCENARIO:
                print(f"  PRN {s.prn:2d}  Δf={s.doppler_hz:+.0f} Hz  "
