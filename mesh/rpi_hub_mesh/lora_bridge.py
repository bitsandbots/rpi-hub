...
2. Run ``rpi-hub-reticulum.service`` (ships with this commit).
3. ``rpi-hub-mesh.service`` auto-connects to the socket within ~5s.
"""
...
class LoraBridge:
    ...
    caller wires to :func:`mesh.rpi_hub_mesh.peers.PeerTable.upsert` and
    ...
    def _read_loop(self, s: socket.socket) -> None:
        ...
        The frame format is whatever ``rpi-hub-reticulum`` chooses to
        emit; the bridge does no parsing here beyond chunking. Parsing
        and signature verification happen in
        :mod:`mesh.rpi_hub_mesh.messages.verify` via the dispatcher.
        """
...
def attach(on_frame: Callable[[bytes], None]) -> LoraBridge:
    """Convenience wrapper used by rpi-hub-mesh.main on startup."""
