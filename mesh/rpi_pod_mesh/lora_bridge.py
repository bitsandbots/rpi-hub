...
2. Run ``rpi-pod-reticulum.service`` (ships with this commit).
3. ``rpi-pod-mesh.service`` auto-connects to the socket within ~5s.
"""
...
RETICULUM_SOCKET = os.environ.get("RPI_POD_RETICULUM_SOCKET", "/run/reticulum/rnsapi.sock")
CONNECT_RETRY_S = 5.0
...
class LoraBridge:
    ...
    caller wires to :func:`mesh.rpi_pod_mesh.peers.PeerTable.upsert` and
    ...
    def _read_loop(self, s: socket.socket) -> None:
        ...
        The frame format is whatever ``rpi-pod-reticulum`` chooses to
        emit; the bridge does no parsing here beyond chunking. Parsing
        and signature verification happen in
        :mod:`mesh.rpi_pod_mesh.messages.verify` via the dispatcher.
        """
...
def attach(on_frame: Callable[[bytes], None]) -> LoraBridge:
    """Convenience wrapper used by rpi-pod-mesh.main on startup."""
