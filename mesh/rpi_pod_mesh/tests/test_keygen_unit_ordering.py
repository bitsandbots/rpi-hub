...
systemd unit's ordering vs. ``rpi-pod-mesh.service`` had no automated
...
KEYGEN_UNIT = REPO_ROOT / "systemd" / "rpi-pod-mesh-keygen.service"
MESH_UNIT = REPO_ROOT / "systemd" / "rpi-pod-mesh.service"
...
def test_keygen_runs_before_mesh() -> None:
    ...
    assert (
        "rpi-pod-mesh.service" in keygen_before
        or "rpi-pod-mesh-keygen.service" in mesh_after
    ), (
        "keygen must be ordered ahead of rpi-pod-mesh.service "
...
def test_mesh_requires_keygen() -> None:
    ...
    happily start rpi-pod-mesh even when keygen failed."""

    mesh = _parse_unit(MESH_UNIT)
    requires = _tokens(mesh["Unit"], "Requires")
    wants = _tokens(mesh["Unit"], "Wants")
    assert (
        "rpi-pod-mesh-keygen.service" in requires
        or "rpi-pod-mesh-keygen.service" in wants
    ), "rpi-pod-mesh.service must Requires= or Wants= the keygen unit"
...
def test_mesh_load_credential_points_at_keygen_output() -> None:
    ...
    assert by_name.get("ed25519_priv") == "/var/lib/rpi-pod/keys/ed25519.priv"
    assert by_name.get("ed25519_pub") == "/var/lib/rpi-pod/keys/ed25519.pub"

    # And the keygen unit really does land its output under
    # /var/lib/rpi-pod/keys (StateDirectory=rpi-pod/keys is the systemd
    # convention for /var/lib/<value>).
    keygen = _parse_unit(KEYGEN_UNIT)
    assert keygen["Service"].get("StateDirectory") == "rpi-pod/keys"
