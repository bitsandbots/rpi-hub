"""Owner-token reader tests for rpi-pod-mesh.

The per-domain split (`/etc/rpi-pod/mesh-owner-token` for peer
trust/block, `/etc/rpi-pod/notes-owner-token` for notes moderation) ships
...
from mesh.rpi_pod_mesh import main
