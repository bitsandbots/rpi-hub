"""rpi-pod-mesh — FastAPI control plane on 127.0.0.1:8500.
...
# install.sh stops being able to land on a token-less /etc/rpi-pod.
OWNER_TOKEN_PATH = Path(os.environ.get("RPI_POD_MESH_TOKEN_FILE", "/etc/rpi-pod/mesh-owner-token"))
LEGACY_OWNER_TOKEN_PATH = Path("/etc/rpi-pod/notes-owner-token")
...
def _on_lora_frame(frame: bytes) -> None:
    ...
    rpi-pod-mesh node. The full dispatcher (notes / presence / index)
...
def _get_bundle() -> identity.IdentityBundle:
    ...
    falls back to ``/var/lib/rpi-pod/keys/`` for dev. The private bytes
    only live in this process's memory — they are not on the filesystem
    from the daemon's point of view (rpi-pod-mesh.service has no
...
def publish_note(body: PublishNoteIn) -> PublishOut:
    ...
    Called by rpi-pod-notes whenever a local user posts. Loopback-only:
    nginx does not expose ``/api/mesh/notes/publish`` (the proxy block
    rewrites only /api/mesh/identity, /peers, /health). rpi-pod-notes
