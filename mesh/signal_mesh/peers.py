...
class PeerTable:
    ...
    def accept_sequence(self, fingerprint: str, seq: int) -> bool:
        ...
        number. If the inbound seq is less than the last seen, we drop
        the message (likely a replay or stale rebroadcast).
        """

        peer = self._by_fp.get(fingerprint)
        if peer is None:
            return True  # first contact; accept and start tracking
