...
systemd unit's ordering vs. ``signal-mesh.service`` had no automated
...
KEYGEN_UNIT = REPO_ROOT / "systemd" / "signal-mesh-keygen.service"
MESH_UNIT = REPO_ROOT / "systemd" / "signal-mesh.service"
...
def test_keygen_runs_before_mesh() -> None:
    ...
    assert (
        "signal-mesh.service" in keygen_before
        or "signal-mesh-keygen.service" in mesh_after
    ), (
        "keygen must be ordered ahead of signal-mesh.service "
...
def test_mesh_requires_keygen() -> None:
    ...
    happily start signal-mesh even when keygen failed."""

    mesh = _parse_unit(MESH_UNIT)
    requires = _tokens(mesh["Unit"], "Requires")
    wants = _tokens(mesh["Unit"], "Wants")
    assert (
        "signal-mesh-keygen.service" in requires
        or "signal-mesh-keygen.service" in wants
    ), "signal-mesh.service must Requires= or Wants= the keygen unit"
...
def test_mesh_load_credential_points_at_keygen_output() -> None:
    ...
    assert by_name.get("ed25519_priv") == "/var/lib/signal/keys/ed25519.priv"
    assert by_name.get("ed25519_pub") == "/var/lib/signal/keys/ed25519.pub"

    # And the keygen unit really does land its output under
    # /var/lib/signal/keys (StateDirectory=signal/keys is the systemd
    # convention for /var/lib/<value>).
    keygen = _parse_unit(KEYGEN_UNIT)
    assert keygen["Service"].get("StateDirectory") == "signal/keys"
