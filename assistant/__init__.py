"""rpi-hub assistant — retrieval-grounded answer service (Phase 6).

Two FastAPI apps live under this package:

* ``rpi_hub_retrieve`` (port 8100) — hybrid BM25 + vector retrieval over the
  workstation-built index in ``/var/lib/rpi-hub/index/``.
* ``rpi_hub_assist`` (port 8200) — wraps retrieval with a safety classifier,
