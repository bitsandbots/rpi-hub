"""Client for rpi-pod-retrieve (the sibling service on 8100).

Goes through localhost rather than calling the retrieval code directly so
the two services have a clean boundary — rpi-pod-retrieve can be
restarted independently and rpi-pod-assist falls back to the no-answer
...
RETRIEVE_ENDPOINT = os.environ.get(
    "RPI_POD_RETRIEVE_ENDPOINT", "http://127.0.0.1:8100/retrieve"
)
RETRIEVE_TIMEOUT_S = float(os.environ.get("RPI_POD_RETRIEVE_TIMEOUT_S", "5.0"))
...
def fetch(query: str, k: int = 6) -> dict[str, object]:
    ...
    No fancy retry — rpi-pod-assist sits behind a 10s end-to-end budget
