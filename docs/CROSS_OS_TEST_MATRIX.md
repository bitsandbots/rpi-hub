...
what the device should do when joined to `RPI-HUB-INFOHUB`.
...
2. Forget any prior `RPI-HUB-INFOHUB` association.
3. Join `RPI-HUB-INFOHUB`.
...
| Sheet opens but page is blank | nginx down or `/var/www/rpi-hub-portal/index.html` missing | `systemctl status nginx`, `nginx -t`, redeploy |
