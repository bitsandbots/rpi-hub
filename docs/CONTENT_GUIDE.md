...
rpi-hub ships with an **empty library** by default. That choice is
...
```bash
ssh pi@hub.local
cd /opt/rpi-hub     # or wherever the repo is checked out
...
source and license in your manifest's `description` field. rpi-hub is MIT
...
| Symptom | Cause | Fix |
|---|---|---|
| `rpi-hub-kiwix` is inactive after install | `/var/lib/kiwix/` is empty | Fetch ZIMs and rsync them in |
| `502 Bad Gateway` at `/library/` | `rpi-hub-kiwix` is down | `systemctl status rpi-hub-kiwix`, look at journal |
