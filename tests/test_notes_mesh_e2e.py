"""End-to-end: ``POST /api/notes`` → ``rpi-hub-mesh /notes/publish``.

The existing tests cover each half in isolation:

* ``notes/rpi_hub_notes/tests/test_publish_mesh_handoff.py`` — notes app
  hands off to ``mesh_client.publish``, and the JSON payload matches
  what ``PublishNoteIn`` expects.
* ``mesh/rpi_hub_mesh/tests/test_mesh.py`` — signing helpers and peer
  table primitives.
...
from mesh.rpi_hub_mesh import identity as mesh_identity
