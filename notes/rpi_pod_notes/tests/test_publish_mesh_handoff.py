...
cross-service hop. A regression in :mod:`rpi_pod_notes.mesh_client`
...
* Monkeypatches :func:`rpi_pod_notes.mesh_client.publish` to capture
   what would have hit ``rpi-pod-mesh /notes/publish``.
 * Asserts the payload shape matches what rpi-pod-mesh's PublishNoteIn
...
from notes.rpi_pod_notes import main, mesh_client, storage
...
def test_post_note_handoff_to_mesh(
    ...
    # asserts (PublishNoteIn in mesh/rpi_pod_mesh/main.py).
...
def test_post_note_handoff_continues_when_mesh_returns_false(
    ...
    """A False return from the mesh client (e.g. rpi-pod-mesh down)
...
def test_mesh_client_payload_shape_via_real_urlopen(
    ...
    bytes — that's the JSON rpi-pod-mesh's pydantic model parses.
