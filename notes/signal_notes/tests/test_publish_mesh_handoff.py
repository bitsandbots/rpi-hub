...
cross-service hop. A regression in :mod:`signal_notes.mesh_client`
...
* Monkeypatches :func:`signal_notes.mesh_client.publish` to capture
  what would have hit ``signal-mesh /notes/publish``.
* Asserts the payload shape matches what signal-mesh's PublishNoteIn
...
from notes.signal_notes import main, mesh_client, storage
...
def test_post_note_handoff_to_mesh(
    ...
    # asserts (PublishNoteIn in mesh/signal_mesh/main.py).
...
def test_post_note_handoff_continues_when_mesh_returns_false(
    ...
    """A False return from the mesh client (e.g. signal-mesh down)
...
def test_mesh_client_payload_shape_via_real_urlopen(
    ...
    bytes — that's the JSON signal-mesh's pydantic model parses.
