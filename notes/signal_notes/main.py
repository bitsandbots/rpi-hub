"""signal-notes — FastAPI app on 127.0.0.1:8400.
...
``/etc/signal/notes-owner-token`` at install time. The token never
...
def post_note(req: Request, body: PostNoteIn, response: Response) -> NoteOut:
    ...
    if not verdict.ok:
        response.headers["Retry-After"] = str(verdict.retry_after_s)
