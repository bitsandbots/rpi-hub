#!/usr/bin/env bash
# rpi-hub — upgrade path validation (v1.1.0 → v1.2.0 → v1.2.1).
#
...
is_disposable_env() {
    ...
    [[ "${rpi_hub_UPGRADE_TEST_ENV:-}" == "disposable" ]] && return 0
    return 1
}
...
if ! is_disposable_env; then
    ...
    log "        VM or container, OR set FORCE=1 / rpi_hub_UPGRADE_TEST_ENV=disposable"
    log "        if you know what you're doing."
    log ""
    log "Manual procedure (one-shot per release):"
    for tag in "${TAGS[@]}"; do
        log "  git checkout ${tag} && sudo ./install.sh"
        log "    systemctl is-active --quiet rpi-hub-{ap,status,notes,mesh}"
        log "    systemctl show -p LoadCredential rpi-hub-mesh.service"
    done
    exit 0
fi
...
UNITS=(rpi-hub-ap rpi-hub-status rpi-hub-notes rpi-hub-mesh)
...
assert_units_active() {
    ...
    for u in "${UNITS[@]}"; do
        if ! systemctl is-active --quiet "${u}.service"; then
            ...
            # rpi-hub-mesh is v1.0+, rpi-hub-notes is v0.9+, rpi-hub-status
...
assert_loadcredential() {
    local tag="$1"
    # v1.2.0 introduced LoadCredential= on rpi-hub-mesh.service.
    # v1.1.0 does NOT carry it. Anything from v1.2.0 onward must.
    if [[ "${tag}" == "v1.1.0" ]]; then
        return 0
    fi
    if ! systemctl show -p LoadCredential rpi-hub-mesh.service \
            | grep -q "owner-token\|LoadCredential="; then
        fatal "${tag}: rpi-hub-mesh.service missing LoadCredential semantics"
    fi
}
