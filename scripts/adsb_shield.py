...
``/etc/rpi-hub/adsb-precision`` (0–4). When the file exists, the
``rpi-hub-adsb-shield.timer`` wakes this script once per second; the
...
shielded copy when present (see ``config/nginx/rpi-hub-portal.conf``).
...
PRECISION_FILE = Path("/etc/rpi-hub/adsb-precision")
DEFAULT_PRECISION = 1
MAX_PRECISION = 4
