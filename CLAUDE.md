...
**rpi-POD** is an Offline Survival InfoHub — a self-contained Wi-Fi
...
| Tag | Phase | Scope |
|-----|-------|-------|
| `v0.1.0-phase1` | Bare AP | hostapd + dnsmasq + rpi-pod-ap.service |
| `v0.2.0-phase2` | Captive portal | nginx default-server 302 + wildcard DNS |
| `v0.3.0-phase3` | Kiwix content | rpi-pod-kiwix.service + `/library/` proxy |
| `v0.4.0-phase4` | Frontend | CoreConduit landing + status page |
| `v0.5.0-phase5` | Status API | FastAPI rpi-pod-status + bake_image.sh + MOTD |
| `v0.6.0-phase6` | RAG assistant | rpi-pod-retrieve + rpi-pod-assist + rpi-pod-llama + Ask UI |
| `v0.9.0-phase9` | Notes board + packs | rpi-pod-notes (tmpfs) + 5 regional packs + `/print/` |
| `v0.8.0-phase8` | Listen | rpi-pod-listen + SAME pipeline + alert banner + Listen UI |
| `v1.0.0`        | Mesh | rpi-pod-mesh + Ed25519 identity + peer trust UI |
...
```
api/rpi_pod_status/   Phase 5 status API (exposes services block since v1.0+)
assistant/           Phase 6 (rpi_pod_retrieve, rpi_pod_assist)
indexer/             Phase 6 workstation-side ZIM → index builder
models/              Phase 6 weight fetch script (workstation only)
listen/rpi_pod_listen/ Phase 8 RTL-SDR control plane + SAME parser
mesh/rpi_pod_mesh/    Phase 7 mesh control plane + Ed25519 identity
notes/rpi_pod_notes/  Phase 9A ephemeral notes board
...
- **Read-only content** — the library is read-only; `rpi-pod-notes` is
...
python -m indexer.build_index --zim-dir /var/lib/kiwix --out ./payload/var/lib/rpi-pod/index
...
- **Phase 7.1** — LoRa data plane. Needs RAK4631 USB hat. Bridge +
  `rpi-pod-reticulum.service` scaffold present.
- **Phase 7.3** — Wi-Fi mesh data plane. Needs second USB Wi-Fi
  adapter. Bridge + `rpi-pod-batman.service` + `scripts/batman_setup.sh`
  scaffold present. Also needs `AF_NETLINK` added to `rpi-pod-mesh`
...
- **`rpi-pod-wizard.html`** — interactive build checklist.
...
- **`archive/Project_rpi-POD_*.docx`** — original engineering specs
...
2. Match the existing unit-of-work pattern: service code under its
   package, systemd unit under `systemd/`, nginx route in
    `config/nginx/rpi-pod-portal.conf`, UI page under `www/portal/`, tile
...
4. If you add an optional capability, surface it in
    `api/rpi_pod_status/system.py:services()` so the status page reflects
   it automatically.
